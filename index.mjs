...
const loggedEvent = async ({ when, what }) => {
	...
	switch (message) {
		...
		case state("target-not-reached"):
			...
			await ask.ask(
				`Enter any key to claim refund`,
				(e => e)
			);
			const didClaim = await ctc.apis.Contributors.tryReclaim();
			if (didClaim) {
				console.log(`Refund successful`);
			} else {
				console.log(`Refund failed, you don't seem to have any`);
			}
			break;
		case state("close-contribution"):
			...
			console.log(
				`The user now has ${stdlib.formatCurrency(bal, 4)} ${stdlib.standardUnit
				}`
			);
			done = true;
			ask.done();
